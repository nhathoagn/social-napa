const Videos = require("../models/reelModel");
...
const reelCtrl = {
  ...
  updatePost: async (req, res) => {
    try {
      ...
      const post = await Videos.findOneAndUpdate(
        { _id: req.params.id },
        {
          content,
          images,
  ...
  likePost: async (req, res) => {
    try {
      ...
      const like = await Videos.findOneAndUpdate(
        { _id: req.params.id },
        {
  ...
  getUserPosts: async (req, res) => {
    try {
      const features = new APIfeatures(
        Videos.find({ user: req.params.id }),
        req.query
      ).paginating();
  ...
  getPost: async (req, res) => {
    try {
      const post = await Videos.findById(req.params.id)
        .populate("user likes", "avatar username fullname followers")
        .populate({
          path: "comments",
          populate: {
  ...
  deletePost: async (req, res) => {
    try {
      const post = await Videos.findOneAndDelete({
        _id: req.params.id,
        user: req.user._id,
      });
